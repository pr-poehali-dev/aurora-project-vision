      ...
      <div className="max-w-[1200px] mx-auto">
          ...
          <div className="flex-1 max-w-lg">
            ...
            <div className="space-y-4 text-foreground">
              <div className="flex items-start gap-3">
                ...
                <p className="text-sm font-mono">RevShare 50–65% от NGR — стабильный пассивный доход без потолка.</p>
              </div>
              <div className="flex items-start gap-3">
                <span className="text-accent mt-1">*</span>
                <p className="text-sm font-mono">СНГ, бурж и крипто ГЕО. Выплаты каждый понедельник.</p>
              </div>
            </div>
          </div>
        ...
        <div id="contact" className="w-full px-6 py-16 flex flex-col md:flex-row items-center justify-center md:justify-between gap-6 md:gap-0 border-t border-border">
          <div className="flex flex-col gap-1 text-center md:text-left">
            ...
            <p className="text-muted-foreground font-mono font-normal text-sm">RevShare 50–65% · CPA 20–100$ · Hybrid</p>
          </div>
          <div className="flex flex-col sm:flex-row gap-3">
            <a href="https://t.me/ecalp" target="_blank" rel="noopener noreferrer">
              <button className="bg-primary text-primary-foreground px-8 py-4 rounded-full font-semibold text-base whitespace-nowrap hover:scale-105 hover:shadow-[0_0_20px_hsl(var(--primary)/0.5)] transition-all duration-300 font-mono flex items-center gap-2">
                @ecalp <Icon name="ArrowUpRight" size={18} />
              </button>
            </a>
            <a href="https://t.me/oyesxa" target="_blank" rel="noopener noreferrer">
              <button className="bg-secondary text-foreground px-8 py-4 rounded-full font-semibold text-base whitespace-nowrap hover:bg-primary hover:text-primary-foreground transition-all duration-300 font-mono flex items-center gap-2">
                @oyesxa <Icon name="ArrowUpRight" size={18} />
              </button>
            </a>
          </div>
        </div>
